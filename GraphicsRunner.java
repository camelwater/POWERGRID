...
//		g.drawString("$ "+game.getUraniumCost(), 1350, 325);
	...
	public void paintOrder(Graphics g, int startX, int startY) //paints player order
	{
		g.setColor(Color.black);
...
//			}
		...
		if(game.getPhase() == 4) 
		{
			...
			if(ppVisible && e.getX() >= 960 && e.getX() <= 995 && e.getY() >= 620 && e.getY() <= 660)
			{
				ppVisible = false;
				repaint();
			}
